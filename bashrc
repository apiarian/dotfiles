...
export PS1="${Red}[${Green} \t \!${Red}] ${Cyan}\u${Red}@${Cyan}\h ${Red}: ${Yellow}\w ${Green}\`git branch 2>/dev/null|grep -e ^*|sed -e 's/* \\(.*\\)/\\git:\\1\\ /'\`${BS}\`if [[ -z \"\\\`git status -z 2>/dev/null\\\`\" ]]; then echo ''; else echo -n '*'; fi\`$VIMTERM\n${Color_Off}"
